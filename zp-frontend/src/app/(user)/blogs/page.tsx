...
// API Functions
async function getBlogs(page: number = 1, search: string = '', sort: string = 'newest'): Promise<ApiResponse> {
  const baseUrl = `${process.env.NEXT_PUBLIC_SERVER_URL}/blogs/fetchBlogs`;
...
async function getFeaturedBlogs(page: number = 1): Promise<ApiResponse> {
  const baseUrl = `${process.env.NEXT_PUBLIC_SERVER_URL}/blogs/fetchFeatured`;
  const query = new URLSearchParams({
    paginate: 'true',
    page: page.toString(),
    limit: '3',
...
// Featured Blogs Carousel Component
function FeaturedBlogsCarousel({ blogs, totalPages, currentPage }: { 
  blogs: Blog[], 
  totalPages: number, 
  currentPage: number 
}) {
...
function BlogsContent() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // Fetch featured blogs and all blogs in parallel
      const [featuredResponse, blogsResponse] = await Promise.all([
        getFeaturedBlogs(featuredPage),
        getBlogs(currentPage, searchTerm, sortOrder)
      ]);
      
      setFeaturedBlogs(featuredResponse.data);
      setFeaturedMeta(featuredResponse.meta);
      setBlogs(blogsResponse.data);
      setMeta(blogsResponse.meta);
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-16 md:py-24">
          <div className="max-w-3xl">
            ...
            <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold text-gray-900 mb-6 leading-tight">
              Our <span className="text-transparent bg-clip-text bg-linear-to-r from-[#8E1B1B] via-[#673AB7] to-[#E91E63]">Blog</span> & Articles
            </h1>
        ...
        <div>
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-gray-600">Browse our complete collection of blog posts</p>
            </div>
            <div className="text-sm text-gray-600">
              Showing {blogs.length} of {meta.totalItems} articles
            </div>
          </div>
                ...
                <div className="max-w-md mx-auto">
                  ...
                  <p className="text-gray-600 mb-6">
                    {searchTerm 
                      ...
                      : 'Start creating blog posts to share insights and stories with the community.'
                    }
                  </p>
                  {searchTerm && (
                    <Link 
              ...
              <div className="bg-linear-to-r from-white to-gray-50 rounded-2xl p-8 border border-gray-100">
                <div className="flex flex-col lg:flex-row items-center justify-between gap-6">
                  ...
                  <div className="flex items-center gap-3">
                    {currentPage > 1 ? (
                      <Link
                        href={`/blogs?page=${currentPage - 1}&search=${searchTerm}&sort=${sortOrder}&featuredPage=${featuredPage}`}
                    ...
                    <div className="flex items-center gap-1">
                      {Array.from({ length: Math.min(5, meta.totalPages) }, (_, i) => {
                        ...
                        return (
                          <Link
                            key={pageNum}
                            href={`/blogs?page=${pageNum}&search=${searchTerm}&sort=${sortOrder}&featuredPage=${featuredPage}`}
                      ...
                      {meta.totalPages > 5 && currentPage < meta.totalPages - 2 && (
                        <>
                          ...
                          <Link
                            href={`/blogs?page=${meta.totalPages}&search=${searchTerm}&sort=${sortOrder}&featuredPage=${featuredPage}`}
